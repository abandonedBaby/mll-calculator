...
# 1. Initialize Default Instruments Table
if "instruments_df" not in st.session_state:
    # We only need the base columns; the others will be calculated automatically
    default_instruments = [
...
# 2. Initialize Weighted Average Values
...
# This runs top-to-bottom every time to generate the calculated fields based on the dataframe
INSTRUMENTS = {}
for _, row in st.session_state.instruments_df.iterrows():
    ...
    # Mathematical logic derived from your spreadsheet
...
def manage_instruments_dialog():
    ...
    # Editable dataframe for instruments
    ...
    if st.button("Save Instruments", type="primary"):
        # Clean up empty rows and save to session state
...
with col1:
    ...
    # Failsafe just in case user deletes all instruments
